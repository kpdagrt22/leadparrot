...
// Bento-grid score breakdown, editorial serif score, mono metadata,
// 1px-border container, no drop shadow.
    ...
    <article className="rounded-none border border-line-2 bg-surface">
      <header className="flex items-start justify-between gap-6 border-b border-line p-6">
        <div className="min-w-0">
          <div className="mb-3 flex items-center gap-2">
            <span className="badge border-accent bg-accent text-accent-fg">{source}</span>
            <span className="font-mono text-[10.5px] uppercase tracking-[0.06em] text-ink-4">
              {subreddit} · {author} · {postedAt}
            </span>
          </div>
          <h3 className="font-display text-2xl leading-tight tracking-tightest text-ink">{title}</h3>
        </div>
        <div className="flex flex-col items-end">
          <span className={cn("font-display text-5xl leading-none tracking-tightest tabular-nums", t.className)}>
            {score}
          </span>
          <span className={cn("mt-1 font-mono text-[10px] uppercase tracking-caps", t.className)}>{t.label}</span>
        </div>
      </header>
      ...
      <div className="grid grid-cols-4 gap-px border-b border-line bg-line">
        {WEIGHTS.map((w) => (
          <div key={w.key} className="bg-surface p-5">
            ...
            <div className="mt-2 font-display text-3xl leading-none tabular-nums text-ink">{scores[w.key]}</div>
      ...
      <div className="flex items-start justify-between gap-6 p-6">
        <p className="max-w-prose text-sm leading-relaxed text-ink-2">
          <span className="mr-2 font-mono text-[10px] uppercase tracking-[0.1em] text-accent">Why</span>
          {reason}
        </p>
        <div className="flex shrink-0 gap-2">
          <button className="btn-secondary">Save</button>
          <button className="btn-primary">Generate reply</button>
        </div>
      </div>
    </article>
  );
